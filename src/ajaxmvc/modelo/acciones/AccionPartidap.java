 ...
 * @version  Ajax-MVC2
 */
public class AccionPartidap implements Accion {
	/**
	...
	public boolean ejecutar(HttpServletRequest request,
		...
		String json, nombre;
		nombre = request.getParameter("nombrenav");
		modelo.setContentType("application/json; charset=UTF-8");
		BeanFotograma fotograma = new BeanFotograma();
		
		if (procesaBD.darPartidap(nombre) != null){		
			
			
		}else{
			fotograma.setError("Error al cargar el partida");
		}
		json = new Gson().toJson(procesaBD.darPartidap(nombre));
