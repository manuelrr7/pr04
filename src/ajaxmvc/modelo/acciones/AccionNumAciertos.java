 ...
 * @version  Ajax-MVC2
 */
public class AccionNumAciertos implements Accion {
	/**
	...
	public boolean ejecutar(HttpServletRequest request,
		...
		BeanAcertado acertado= new BeanAcertado(user);
		
		if (procesaBD.aciertosTotal(user) != null){		
			

		}else{
			acertado.setError("Error al cargar el num. de aciertos");
		}
		json = new Gson().toJson(procesaBD.aciertosTotal(user));
