...
import ajaxmvc.modelo.beans.BeanAcertado;
 ...
 * @version  Ajax-MVC2
 */
public class AccionRegistraAci implements Accion {
	/**
	...
	public boolean ejecutar(HttpServletRequest request,
		...
		String user, json, titPelicula;
		
		user = request.getParameter("user");
		titPelicula = request.getParameter("titPelicula");
		System.out.println("registra: user "+user+" titulo "+titPelicula);
		modelo.setContentType("application/json; charset=UTF-8");
		
		if ((titPelicula!=null) && (titPelicula!="") && (user!=null) && (user!=""))
		{

		BeanAcertado acertado = new BeanAcertado();
		
		if (procesaBD.registrarFoto(titPelicula, user)){		
			
		
			
		}else{
			acertado.setError("Error al registrar fotograma acertado");
		}
		
		json = new Gson().toJson(acertado);
