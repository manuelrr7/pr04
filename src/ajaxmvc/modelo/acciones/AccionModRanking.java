...
import ajaxmvc.modelo.beans.Ranking;
 ...
 * @version  Ajax-MVC2
 */
public class AccionModRanking implements Accion {
	/**
	...
	public boolean ejecutar(HttpServletRequest request,
		...
		String login, json, puntos;
		
		login = request.getParameter("login");
		puntos = request.getParameter("puntos");
		System.out.println("MODIFICA PUNTOS "+puntos+" usuarios "+login);
		modelo.setContentType("application/json; charset=UTF-8");
		
		if ((login!=null) && (login!="") && (puntos!=null) && (puntos!=""))
		{
		
		Ranking ranking = new Ranking(login, puntos);
		
		if (procesaBD.modificaRanking(login, puntos)){		
			
		
			
		}else{
			ranking.setError("No se ha podido modificar el ranking");
		}
		
		json = new Gson().toJson(ranking);
